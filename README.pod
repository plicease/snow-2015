...
way of searching for libraries in the usual system locations.  We have a 
mixed environment of OS X and Linux for toy deliveries and testing, so 
it this had to work on both platforms.  Although L<FFI::CheckLib> 
generally works well in Windows, Linux, OS X (and others), the OpenGL 
libraries on OS X are part of what they call a "framework", which isn't 
