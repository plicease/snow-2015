...
sync with functions as we add them is very tedius, so instead we use 
...
bottom of the module because otherwise C<%GL::> won't be populated 
completely, until after all of the C<attach> methods are called.
